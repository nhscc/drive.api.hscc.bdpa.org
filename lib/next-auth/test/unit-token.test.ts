 ...
 */
function objectIdPseudoSortPredicate(order: 'ascending' | 'descending') {
...
describe('::createToken', () => {
  it('creates an auth entry and returns the new token', async () => {
    ...
    const crypto = jest.requireActual('node:crypto');
    const incomingToken1 = crypto.randomUUID();
    const incomingToken2 = crypto.randomUUID();

    mockRandomUUID.mockReturnValueOnce(incomingToken1);
    mockRandomUUID.mockReturnValueOnce(incomingToken2);
  ...
  it('rejects if passed invalid data', async () => {
    ...
    const errors: [params: Parameters<typeof createToken>, error: string][] = [
      [
...
describe('::deleteTokenById', () => {
  ...
  it('rejects if passed invalid data', async () => {
    ...
    const errors: [params: Parameters<typeof deleteTokenById>, error: string][] = [
      [
        [{} as unknown as Parameters<typeof deleteTokenById>[0]],
        expect.stringContaining('irreducible')
      ],
      [[{ auth_id: undefined }], expect.stringContaining('irreducible')],
      [
      ...
      [[{ auth_id: {} as unknown as ObjectId }], expect.stringContaining('irreducible')],
      [[{ auth_id: '' }], expect.stringContaining('irreducible')],
      [[{ auth_id: 'xyz' }], expect.stringContaining('irreducible')],
      [[{ auth_id: 5 as unknown as ObjectId }], expect.stringContaining('irreducible')]
    ];
...
describe('::getTokensByAttribute', () => {
  it('returns array of all auth entries with matching attributes', async () => {
    ...
    const incomingAuthEntry1: InternalAuthEntry = {
      _id: new ObjectId(),
      deleted: false,
    ...
    const incomingAuthEntry2: InternalAuthEntry = {
      _id: new ObjectId(),
      deleted: false,
    ...
    await (await getAuthDb()).insertMany([incomingAuthEntry1, incomingAuthEntry2]);

    await expect(
      getTokensByAttribute({ filter: { owner: owners } })
    ).resolves.toStrictEqual(
      [...entries, incomingAuthEntry1, incomingAuthEntry2]
  ...
  it('rejects if passed invalid data', async () => {
    ...
    type Params = Parameters<typeof getTokensByAttribute>;
    const errors: [params: Params, error: string][] = [
      [
...
describe('::updateTokenAttributesById', () => {
  ...
  it('rejects if passed invalid data', async () => {
    ...
    const errors: [
      params: Parameters<typeof updateTokenAttributesById>,
      error: string
    ][] = [
      [[{} as any], ErrorMessage.InvalidSecret('update')],
      [[{ auth_id: undefined, update: {} }], expect.stringContaining('irreducible')],
      [[{ auth_id: null as any, update: {} }], expect.stringContaining('irreducible')],
      [[{ auth_id: false as any, update: {} }], expect.stringContaining('irreducible')],
      [[{ auth_id: true as any, update: {} }], expect.stringContaining('irreducible')],
      [[{ auth_id: {} as any, update: {} }], expect.stringContaining('irreducible')],
      [[{ auth_id: 'xyz123', update: {} }], expect.stringContaining('irreducible')],
...
describe('::updateTokensAttributesByAttribute', () => {
  ...
  it('rejects if passed invalid data', async () => {
    ...
    const errors: [
      params: Parameters<typeof updateTokensAttributesByAttribute>,
      error: string
    ][] = [
