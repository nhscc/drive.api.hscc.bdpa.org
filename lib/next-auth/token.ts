...
import { validAuthenticationSchemes } from 'multiverse/next-auth/authenticate';
import { getConfig } from 'multiverse/next-auth/constants';
...
import type { AuthenticationScheme } from 'multiverse/next-auth/authenticate';
...
const debug = createDebugLogger({ namespace: 'next-auth:token' });
  ...
  if (authString !== undefined) {
    ...
    if (scheme === 'bearer') {
      ...
      throw new AppValidationError(
        `auth string handler for scheme "${String(scheme)}" is not implemented`
      );
    }
    ...
    debug(`deriving token of scheme "${String(maybeScheme)}" from auth data`);
    ...
    if (scheme === 'bearer') {
      ...
      throw new AppValidationError(
        `auth data handler for scheme "${String(scheme)}" is not implemented`
      );
    }
  } else {
...
export async function createToken({
  ...
  if (isNewAuthEntry(data)) {
    const incomingToken: InternalAuthBearerEntry = {
      _id: new ObjectId(),
      deleted: false,
    ...
    try {
      await (await getAuthDb()).insertOne({ ...incomingToken });
    } catch (error) {
    ...
    return toPublicAuthEntry(incomingToken);
  } else {
